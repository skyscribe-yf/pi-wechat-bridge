...
```
企业微信 App → 企业微信服务器 → pi-wechat-bridge → pi (RPC mode, per-user)
                                            ↓
                                     目标代码目录 (PI_CWD)
```
...
| `src/tunnel.js` | Cloudflared tunnel 管理，自动获取公网 URL 并通知管理员。 |
| `scripts/watchdog.sh` | 进程守护脚本，基于 /health 端点检测，per-user busy 卡住检测。 |
...
- `UserSession` 封装了 `PiRpcClient`、busy 锁、busyTimer、streamBuffer
...
pi 以 `--mode rpc` 启动，JSONL 协议通信。`_dispatchProgress` 把进度事件投递给 `onProgress`。
